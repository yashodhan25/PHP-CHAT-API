...
$select = "SELECT * FROM `conversations` WHERE `receiver` IN ('$sender', '$receiver') AND `sender` IN ('$sender', '$receiver') ";
...
if($executeselect != false){
    ...
    if($row != 0){
        ...
        echo json_encode(['message'=>'got all successfully', 'data'=>[], 'success'=>true]);
    }
}else{
